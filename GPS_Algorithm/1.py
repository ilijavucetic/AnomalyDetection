    ...
    for val in misOrderedList:
        ...
        for seq in seqList:
            ...
            while seqNoSpace.find(val[0]) != -1:
                ...
                if (seqNoSpace[seqNoSpace.find(val[0]) + len(val[0])] == '}' or seqNoSpace[
                        seqNoSpace.find(val[0]) + len(val[0])] == ',') and (
                        seqNoSpace[seqNoSpace.find(val[0]) - 1] == '{' or seqNoSpace[
                        seqNoSpace.find(val[0]) - 1] == ','):
                    count = count + 1
                    break
                else:
...
def level2_candidate_gen(L, pi, supportcount, misDict):
    ...
    for item in L:
        if float(supportcount[item] / 71) >= float(misDict[item]):
            ...
            for tmpItem in temp:
                if float(supportcount[tmpItem] / 71) >= float(misDict[item]) and abs(
                                float(supportcount[item] / 71) - float(supportcount[tmpItem] / 71)) <= 0.05:
